...
const dbName = process.env.DB_NAME || process.env.MYSQLDATABASE || 'el_progreso';
...
export const validateEnv = () => {
  ...
  if (isProduction) {
    ...
    if (!process.env.CORS_ORIGIN && !process.env.CORS_ORIGINS) {
      ...
      for (const origin of corsOrigins) {
        if (!/^https:\/\//i.test(origin)) {
          errors.push(`CORS_ORIGIN debe usar HTTPS en producción (${origin}).`);
        }
      }
    }
    ...
    if (!adminPassword) {
      errors.push('ADMIN_PASSWORD es obligatorio en producción (se usa en db:setup).');
    } else if (adminPassword.length < 12) {
      errors.push('ADMIN_PASSWORD debe tener al menos 12 caracteres en producción.');
    } else if (INSECURE_ADMIN_PASSWORDS.has(adminPassword)) {
      errors.push('ADMIN_PASSWORD no puede ser un valor de desarrollo.');
    }
  ...
  if (errors.length > 0) {
    ...
    console.error('\nRevisá backend/.env.example y DEPLOY.md\n');
    process.exit(1);
  }
};
